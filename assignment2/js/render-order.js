...
const shopping__cart_items = document.getElementById("shopping__cart-items");
const total_cart = document.getElementById("shopping__cart-total");
...
function render_cart(cart) {
    ...
    cart.forEach((order, index) => {
        ...
        order_details_html +=  `
            <input type="hidden" name="${order["product_name"]}" value="${order["order_quantity"]}">
        `;

    });
    order_details.innerHTML = order_details_html;
    shopping__cart_items.innerHTML = shopping_cart_html;
    render_total();
}
